...
#numpy implementation of the linformer attention mechanism
@dace.program
def linformer_mha_forward(
    q: dace_dtype[B, SN, N],
    k: dace_dtype[B, SM, N],
    v: dace_dtype[B, SM, N],
    wq: dace_dtype[P, H, N],
    ...
    #Assumptions: Dimension must be divisible by the number of heads
    qq = np.einsum("phi,bji->phbj", wq, q) + bq
    kk = np.einsum("phi,bki->phbk", wk, k) + bk
    vv = np.einsum("phi,bki->phbk", wv, v) + bv
